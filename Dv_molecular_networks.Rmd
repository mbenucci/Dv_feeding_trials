...
One season across 3 lakes: Wroxham Broad, Grafham Water and Rockland Broad (control site).
...
guts[guts$predator_id == "Gzad","Gammarus_zaddachi"] = 0
guts[guts$predator_id == "Gtig","Gammarus_tigrinus"] = 0
guts[guts$predator_id == "Cmultise","Corophium_multisetosum"] = 0
...
adonis(dist.guts~fact_lake*fact_pred*fact_site, permutations=9999, method="euclidean")
```
...
ordispider(meta.guts,group=fact_lake[,1], col=c("blue","orange","black"),label=T)
...
abund.edna[,!c(colnames(abund.edna) %in% c("sample_id","sample_type","site","lake"))]/6
```
...
```{r permanova guts reads, include=T, echo=F}
adonis.edna = as.data.frame(abund.edna[,!c(colnames(abund.edna) %in% c("sample_id","sample_type","site","lake"))]/6)
adonis.edna = bind_cols(adonis.edna, abund.edna[,c(colnames(abund.edna) %in% c("sample_id","sample_type","site","lake"))])

fact_site.edna = matrix(c(adonis.edna$site),ncol=1)
fact_lake.edna = matrix(c(adonis.edna$lake),ncol=1)

adonis.edna = adonis.edna[,!c(colnames(adonis.edna) %in% c("sample_id","sample_type","site","lake"))]

dist.edna = vegdist(adonis.edna)
adonis(dist.edna~fact_lake.edna*fact_site.edna, permutations = 9999, method="jaccard")
```
...
points(meta.edna, select=which(fact_lake.edna[,1]=="WB"), col="black",pch=19)
points(meta.edna, select=which(fact_lake.edna[,1]=="GW"), col="blue",pch=15)
points(meta.edna, select=which(fact_lake.edna[,1]=="RB"), col="red",pch=17)
ordispider(meta.edna,group=fact_lake.edna[,1], col=c("blue","orange","black"),label=T)
ordiellipse(meta.edna,group=fact_lake.edna[,1],kind="ehull", lty=c(1,3,5),label=F,lwd=2)
...
fact_type.edki = matrix(c(abund.edna.kick$sample_type), ncol=1)
fact_site.edki = matrix(c(abund.edna.kick$site), ncol=1)
fact_lake.edki = matrix(c(abund.edna.kick$lake), ncol=1)
...
adonis(dist.edna.kick~fact_lake.edki*fact_site.edki*fact_type.edki, permutations=9999, method="jaccard")
```
...
points(meta.edki, select=which(fact_lake.edki[,1]=="WB"), col="black",pch=19)
points(meta.edki, select=which(fact_lake.edki[,1]=="GW"), col="blue",pch=15)
points(meta.edki, select=which(fact_lake.edki[,1]=="RB"), col="red",pch=17)
ordispider(meta.edki,group=fact_lake.edki[,1], col=c("blue","orange","black"),label=T)
ordiellipse(meta.edki,group=fact_type.edki[,1],kind="ehull", lty=c(1,5),label=F,lwd=2)
...
fact_type.full = matrix(c(abund.edna.kick.guts$sample_type), ncol=1)
fact_site.full = matrix(c(abund.edna.kick.guts$site), ncol=1)
fact_lake.full = matrix(c(abund.edna.kick.guts$lake), ncol=1)
...
adonis(dist.full~fact_lake.full*fact_site.full*fact_type.full, permutations=9999, method="euclidean")
```

```{r PERMANOVA on eDNA, kick and guts}
meta.full = metaMDS(dist.full, distance="jaccard", trymax=200)
```
...
points(meta.full, select=which(fact_lake.full[,1]=="WB"), col="black",pch=19)
points(meta.full, select=which(fact_lake.full[,1]=="GW"), col="blue",pch=15)
points(meta.full, select=which(fact_lake.full[,1]=="RB"), col="red",pch=17)
ordispider(meta.full,group=fact_lake.full[,1], col=c("blue","orange","black"),label=T)
ordiellipse(meta.full,group=fact_type.full[,1],kind="ehull", lty=c(1,3,5),label=T,lwd=2)
...
ggplot(trials.melt[trials.melt$variable != "D.villosus",], aes(times, value)) +
  geom_boxplot(outlier.shape=NA, aes(size=6)) +
  geom_jitter(alpha = 0.4, aes(size=10)) +
  facet_grid(variable~., scale="free") +
  theme_bw() +
  labs(x="Digestion time (hours)",y="Normalised DNA reads") +
...
```{r NMDS publication}
png("R_images/publication_NMDS.png", width=5000, height=3000, units="px", res=300)
lay.matrix = matrix(c(1,2,3,4,5,6), ncol=3, nrow=2, byrow=T)
layout(lay.matrix, heights=c(1,1), widths=c(2,2,2))

# a) gut contents only
plot(meta.guts,type="n", xlim=c(-2,2), ylim=c(-1,1), cex.axis=2, cex.lab=1.5)
ordispider(meta.guts,group=fact_lake[,1], col=c("blue","orange","black"),label=T, cex=2)
points(meta.guts, select=which(fact_lake[,1]=="WB"), col="black",pch=19,cex=2)
points(meta.guts, select=which(fact_lake[,1]=="GW"), col="blue",pch=15,cex=2)
points(meta.guts, select=which(fact_lake[,1]=="RB"), col="red",pch=17,cex=2)
ordiellipse(meta.guts,group=fact_pred[,1],kind="ehull", lty=c(1,5),label=F,lwd=1)
text(1.5,-1.5, label=paste("Stress:", round(meta.guts$stress, 4)), cex=1.5)
text(-1.9,1.45, label="a)", cex=3)
text(0,1.45, label="guts", cex=2.5)
text(-1.8,0, label="Dvil", cex=2.5)
text(1.5,1, label="Gzad", cex=2.5)


# b) eDNA only
plot(meta.edna,type="n", xlim=c(-0.5,0.5), ylim=c(-0.5,0.5), cex.axis=2, cex.lab=1.5)
points(meta.edna, select=which(fact_lake.edna[,1]=="WB"), col="black",pch=19, cex=2)
points(meta.edna, select=which(fact_lake.edna[,1]=="GW"), col="blue",pch=15, cex=2)
points(meta.edna, select=which(fact_lake.edna[,1]=="RB"), col="red",pch=17, cex=2)
ordispider(meta.edna,group=fact_lake.edna[,1], col=c("blue","orange","black"),label=T, cex=2)
ordiellipse(meta.edna,group=fact_lake.edna[,1],kind="ehull", lty=c(1,3,5),label=F,lwd=1)
text(0.45,-0.45, label=paste("Stress:", round(meta.edna$stress, 4)), cex=1.5)
text(-0.6,0.45, label="b)", cex=3)
text(-0.2,0.45, label="eDNA", cex=2.5)

# c) kick samples only
plot(meta.kick, type="n", xlim=c(-0.5,0.5), ylim=c(-0.5,0.5), cex.axis=2, cex.lab=1.5)
ordispider(meta.kick, group=fact_lake.kick[,1],col=c("blue","orange","green"),label=T, cex=2)
points(meta.kick, select=which(fact_lake.kick[,1]=="WB"),col="black",pch=20, cex=2)
points(meta.kick, select=which(fact_lake.kick[,1]=="GW"),col="blue",pch=15, cex=2)
points(meta.kick, select=which(fact_lake.kick[,1]=="RB"),col="red",pch=17, cex=2)
ordiellipse(meta.kick, group=fact_lake.kick[,1],kind="ehull",lty=c(1,3,5),label=F,lwd=1)
text(0.5,-0.45, label=paste("Stress:", round(meta.kick$stress, 4)), cex=1.5)
text(-0.6,0.45, label="c)", cex=3)
text(0,0.45, label="kick", cex=2.5)

# d) kick and eDNA
plot(meta.edki,type="n", xlim=c(-0.5,0.5), ylim=c(-0.5,0.5), cex.axis=2, cex.lab=1.5)
points(meta.edki, select=which(fact_lake.edki[,1]=="WB"), col="black",pch=19, cex=2)
points(meta.edki, select=which(fact_lake.edki[,1]=="GW"), col="blue",pch=15, cex=2)
points(meta.edki, select=which(fact_lake.edki[,1]=="RB"), col="red",pch=17, cex=2)
ordispider(meta.edki,group=fact_lake.edki[,1], col=c("blue","orange","black"),label=T, cex=2)
ordiellipse(meta.edki,group=fact_type.edki[,1],kind="ehull", lty=c(1,5),label=F,lwd=1)
text(0.5,-0.45, label=paste("Stress:", round(meta.edki$stress, 4)), cex=1.5)
text(-0.6,0.45, label="d)", cex=3)
text(0.5,0.4, label="eDNA", cex=2.5)
text(-0.5,0.25, label="kick", cex=2.5)

# e) kick, eDNA and guts
plot(meta.full,type="n", xlim=c(-4,3), ylim=c(-4,3), cex.axis=2, cex.lab=1.5)
points(meta.full, select=which(fact_lake.full[,1]=="WB"), col="black",pch=19, cex=2)
points(meta.full, select=which(fact_lake.full[,1]=="GW"), col="blue",pch=15, cex=2)
points(meta.full, select=which(fact_lake.full[,1]=="RB"), col="red",pch=17, cex=2)
ordispider(meta.full,group=fact_lake.full[,1], col=c("blue","orange","black"),label=T, cex=2)
ordiellipse(meta.full,group=fact_type.full[,1],kind="ehull", lty=c(1,3,5),label=F,lwd=1)
text(3,-3.8,label=paste("Stress:", round(meta.full$stress, 4)), cex=1.5)
text(-4.7,2.7, label="e)", cex=3)
text(1.7,2.5, label="eDNA", cex=2.5)
text(-4,1.5, label="guts", cex=2.5)
text(1.5,-2.5, label="kick", cex=2.5)

# Common legend
plot.new()
legend("left", legend=c("Grafham Water","Wroxham Broad","Rockland Broad"), col=c("blue","black","red"), pch=c(15,19,17), cex=2, ncol=1)
dev.off()
```
