...
boxplot(colSums(feb_reads), colSums(aug_reads), ylab="N reads", xlab="Data frames", main = "Read depth of D.villosus samples", names=c("dv_feb\n(Feb2017)", "dv_aug\n(Aug2017)"), las=1, cex.axis=0.8)
```
...
feb_cover = subset(feb_reads, select = colSums(feb_reads) >= 2000)
...
(count(colSums(feb_cover) > 0)[2]/count(colSums(feb_reads) > 0)[2])*100
(count(colSums(aug_cover) > 0)[2]/count(colSums(aug_reads) > 0)[2])*100
```
...
discard.feb = c("Hymenoptera","Thalassiosira_pseudonana","unassigned")

feb.clean = feb_samples[!rownames(feb_samples) %in% discard.feb,]

# Second frame
aug_samples = subset(aug_samples, subset = rowSums(aug_samples) > 0)
#rownames(aug_samples)

discard.aug = c("Abramis_brama","Anser","Aphanomyces","Arthropoda_environmental_sample","Ascomorpha_ecaudis","Asplanchna_sieboldi","Brachionus","Brachionus_angularis","Brachionus_calyciflorus","Chromulinales","Chrysophyceae_sp.","Cochliopodium","Cochliopodium_actinophorum","Cochliopodium_sp._SG-2014","Cyclostephanos_dubius","Cyclostephanos_invisitatus","Cyclostephanos_sp._WTC16","Didymella_pinodes","Dinobryon","Dinobryon_divergens","Dinophyceae","Dissotrocha_macrostyla","Encyonema","Euchlanis_dilatata","Eukaryota","Fistulifera_solaris","Fusarium","Gomphonema_parvulum","Gymnocephalus","Hymenoptera","Ilyonectria_destructans","invertebrate_environmental_sample","Kellicottia_sp._BIOUG30062-G11","Keratella","Keratella_cochlearis","Keratella_quadrata","Lagenidium","Leptolegnia_sp._BOLD:AAX5717","Mallomonas_sp.","Melosira_ambiqua","Metazoa","Oncorhynchus_mykiss","Oomycetes","Paraphysomonas_sp.","Penicillium_sclerotiorum","Phytophthora","Ploima","Pseudorasbora_parva","Pythium","Pythium_marsipium","Saprolegnia_delica","Saprolegniaceae","Sellaphora_obesa","Skeletonema_potamos","Sordariomycetes","Stephanodiscus","Stephanodiscus_cf._minutulus","Tetracladium","Tetradesmus_obliquus","Thalassiosira_pseudonana","unassigned","Uncinais_uncinata","uncultured_Pythium","Urocentrum_turbo","Woloszynskia","Yamagishiella_unicocca","zooplankton_environmental_sample")

aug.clean = aug_samples[!rownames(aug_samples) %in% discard.aug,]
```

We retained approx. 87.4%, and 76.3% of the initial N reads.

```{r retained reads and samples, include=T}
(sum(feb.clean)/sum(feb_reads))*100
(sum(aug.clean)/sum(aug_reads))*100

((sum(feb_cover)/sum(feb_reads))*100-(sum(feb.clean)/sum(feb_reads))*100)
((sum(aug_cover)/sum(aug_reads))*100-(sum(aug.clean)/sum(aug_reads))*100)
```

# Calculating contamination threshold
## First data frame

First data frame containes data from the sequencing run from February 2017, which includes feeding trials of _Dikerogammarus villosus_. To calculate the contamination threshold we decided to account for all laboratory contaminations (e.g. Human DNA) in our `POSITIVE` samples.

```{r threshold first frame, include=T}
## Positive samples from `feb.clean` (August 2017)
grep("POS", colnames(feb.clean))

feb.pos = feb.clean[colnames(feb.clean) == "POSITIVE.3"]
feb.pos[,]/colSums(feb.pos)
rownames(feb.pos)

feb.pre.filter = as.data.frame(t(feb.clean))
...
feb.clean = as.data.frame(t(feb.clean))
feb.clean[][feb.clean[,]/rowSums(feb.clean) <= thres] <- 0

## Writing pre- and post-filter data to output
write.csv(feb.clean, "R_outputs/DvFeb_cleaned_reads.csv")
...
Contamination from _Dikerogammarus villosus_ stands at 0.04%, _Gammarus zaddachi_ at 0.02%, and _Homo sapiens_ at 0.026%. Based on the levels of these contaminations we decided to apply a threshold of 0.03% despite this will maintain some levels of contamination from _D. villosus_ DNA.

```{r threshold first frame, include=T}
## Positive samples from `feb.clean` (August 2017)
aug.clean[,grep("POS", colnames(aug.clean))]

aug.pos = aug.clean[,grep("POS", colnames(aug.clean))]
...
aug.pre.filter = as.data.frame(t(aug.clean))
...
aug.clean = as.data.frame(t(aug.clean))
aug.clean[][aug.clean[,]/rowSums(aug.clean) <= thres] <- 0


## Writing pre- and post-filter data to output
write.csv(aug.clean, "R/DvAug_cleaned_reads.csv")
write.csv(aug.pre.filter, "R/DvAug_pre-filter.csv")
write.csv(aug.post.filter, "R/DvAug_post-filter.csv")
```
