...
aug_cover = subset(aug_reads, select = colSums(aug_reads) >= 500)
```
...
Read survival is set at 99.7% for the first library and 99.9% for the second.
...
## Second frame
#colnames(aug_cover)
aug_samples = aug_cover[,-c(grep("Ha|CH|RB", colnames(aug_cover)))]
```
...
discard.aug = c("Abramis_brama","Anser","Aphanomyces","Arthropoda_environmental_sample","Chromulinales","Cochliopodium","Cochliopodium_actinophorum","Cochliopodium_sp._SG-2014","Cyclostephanos_dubius","Cyclostephanos_invisitatus","Cyclostephanos_sp._WTC16","Dinophyceae","Encyonema","Eukaryota","Fistulifera_solaris","Gomphonema_parvulum","Gymnocephalus","Hominidae","Homo_sapiens","Ilyonectria_destructans","invertebrate_environmental_sample","Mallomonas_sp.","Melosira_ambiqua","Metazoa","Mus_musculus","Oncorhynchus_mykiss","Oomycetes","Paraphysomonas_sp.","Penicillium_sclerotiorum","Phytophthora","Pinnularia_neomajor","Pseudorasbora_parva","Pythium","Saprolegnia_delica","Saprolegniaceae","Sellaphora_obesa","Skeletonema_potamos","Stephanodiscus","Tetracladium","Tetradesmus_obliquus","Thalassiosira_pseudonana","unassigned","Yamagishiella_unicocca","Woloszynskia","zooplankton_environmental_sample")

aug_clean = aug_samples[!rownames(aug_samples) %in% discard.aug,]
```

We retained approx. 88.6%, and 49.4% of the initial N reads.
...
dvc$sample_id = gsub("[.][0-9]{1}$", "", gsub("[.][0-9AB]{1,2}$", "", dvc$sample_id))
...
We plot now the detection of preys by PCR replicates and trials replicates.

```{r pcr replicate plot for feeding trials}
dva$trial_id = gsub("^[A-Za-z]{3}", "", gsub("C${1}","", dva$sample_id))
dvd$trial_id = gsub("^[A-Za-z]{3}", "", gsub("C${1}","", dvd$sample_id))
dvc$trial_id = gsub("^[A-Za-z]{3}", "", gsub("[.]C${1}","", dvc$sample_id))

#colnames(dva)
#dva$trial_repl

#stat_summary(fun.y='mean', geom='text', aes(label=signif(..y..,4),x=as.integer(tag)))+ 
#stat_summary(fun.y="fun0",fun.ymin='sd', geom='text', aes(label=paste("sd:",signif(..ymin..,2)),x=as.integer(tag)))+

fa = dva %>% mutate(trial_id = factor(trial_id, levels=c("0","4","8","12","24","36"))) %>% mutate(trial_repl = factor(trial_repl, levels=c("control","1","2","3","4","5"))) %>%
  ggplot(aes(pcr_repl, Asellus_aquaticus)) +
  geom_point() +
  labs(x="PCR replicates", y="A. aquaticus DNA reads") +
  facet_grid(trial_id~trial_repl) +
  theme_bw()

fd = dvd %>% mutate(trial_id = factor(trial_id, levels=c("0","4","8","12","24","36"))) %>% mutate(trial_repl = factor(trial_repl, levels=c("control","1","2","3","4","5"))) %>%
  ggplot(aes(pcr_repl, Daphnia_magna)) +
  geom_point() +
  labs(x="PCR replicates", y="D. magna DNA reads") +
  facet_grid(trial_id~trial_repl) +
  theme_bw()

fc = dvc %>% mutate(trial_id = factor(trial_id, levels=c("0","4","8","12","24","36"))) %>% mutate(trial_repl = factor(trial_repl, levels=c("control","1","2","3","4","5"))) %>%
  ggplot(aes("1", Crangonyx_pseudogracilis)) +
  geom_point() +
  labs(x="PCR replicates", y="C. pseudogracilis DNA reads") +
  facet_grid(trial_id~trial_repl) +
  theme_bw()

png("R_images/pcr_replicates_feeding_trials.png", width=4000, height=4000, units="px", res=300)
plot_grid(fa, fd, fc, labels=c("a)","b)","c)"), nrow=1, ncol=3)
dev.off()
```


Distribution of DNA reads across PCR replicates

```{r PCR replicates distribution Asellus}
# Asellus
print("DvA - repl 1")
mean(dva[c(dva$trial_repl == 1 & dva$trial_id == "0"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 1 & dva$trial_id == "0"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 1 & dva$trial_id == "0"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 1 & dva$trial_id == "4"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 1 & dva$trial_id == "4"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 1 & dva$trial_id == "4"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 1 & dva$trial_id == "8"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 1 & dva$trial_id == "8"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 1 & dva$trial_id == "8"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 1 & dva$trial_id == "12"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 1 & dva$trial_id == "12"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 1 & dva$trial_id == "12"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 1 & dva$trial_id == "24"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 1 & dva$trial_id == "24"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 1 & dva$trial_id == "24"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 1 & dva$trial_id == "36"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 1 & dva$trial_id == "36"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 1 & dva$trial_id == "36"), "Asellus_aquaticus"]))

# Asellus replic 2
print("DvA repl 2")
mean(dva[c(dva$trial_repl == 2 & dva$trial_id == "0"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 2 & dva$trial_id == "0"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 2 & dva$trial_id == "0"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 2 & dva$trial_id == "4"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 2 & dva$trial_id == "4"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 2 & dva$trial_id == "4"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 2 & dva$trial_id == "8"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 2 & dva$trial_id == "8"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 2 & dva$trial_id == "8"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 2 & dva$trial_id == "12"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 2 & dva$trial_id == "12"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 2 & dva$trial_id == "12"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 2 & dva$trial_id == "24"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 2 & dva$trial_id == "24"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 2 & dva$trial_id == "24"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 2 & dva$trial_id == "36"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 2 & dva$trial_id == "36"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 2 & dva$trial_id == "36"), "Asellus_aquaticus"]))

# Asellus replic 4
print("DvA repl 4")
mean(dva[c(dva$trial_repl == 4 & dva$trial_id == "0"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 4 & dva$trial_id == "0"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 4 & dva$trial_id == "0"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 4 & dva$trial_id == "4"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 4 & dva$trial_id == "4"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 4 & dva$trial_id == "4"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 4 & dva$trial_id == "8"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 4 & dva$trial_id == "8"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 4 & dva$trial_id == "8"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 4 & dva$trial_id == "12"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 4 & dva$trial_id == "12"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 4 & dva$trial_id == "12"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 4 & dva$trial_id == "24"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 4 & dva$trial_id == "24"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 4 & dva$trial_id == "24"), "Asellus_aquaticus"]))

mean(dva[c(dva$trial_repl == 4 & dva$trial_id == "36"), "Asellus_aquaticus"])
sd(dva[c(dva$trial_repl == 4 & dva$trial_id == "36"), "Asellus_aquaticus"])/sqrt(length(dva[c(dva$trial_repl == 4 & dva$trial_id == "36"), "Asellus_aquaticus"]))
```

```{r PCR replicates distribution Daphnia}
# Daphnia
print("DvD repl 2")
mean(dvd[c(dvd$trial_repl == 2 & dvd$trial_id == "4"), "Daphnia_magna"])
sd(dvd[c(dvd$trial_repl == 2 & dvd$trial_id == "4"), "Daphnia_magna"])/sqrt(length(dvd[c(dvd$trial_repl == 2 & dvd$trial_id == "4"), "Daphnia_magna"]))
```
...
dvd.info = dvd[,c("sample_id","sample_type","trial_repl","pcr_repl","trial_id")]
dva.info = dva[,c("sample_id","sample_type","trial_repl","pcr_repl","trial_id")]
dvc.info = dvc[,c("sample_id","sample_type","trial_repl","pcr_repl","trial_id")]

dvd.norm = (dvd[,!c(colnames(dvd) %in% c("sample_id","sample_type","trial_repl","pcr_repl","trial_id"))]/sum(feb_clean[,!c(colnames(feb_clean) %in% c("sample_id","sample_type","trial_id"))]))*1000000

dva.norm = (dva[,!c(colnames(dva) %in% c("sample_id","sample_type","trial_repl","pcr_repl","trial_id"))]/sum(feb_clean[,!c(colnames(feb_clean) %in% c("sample_id","sample_type","trial_id"))]))*1000000

dvc.norm = (dvc[,!c(colnames(dva) %in% c("sample_id","sample_type","trial_repl","pcr_repl","trial_id"))]/sum(aug_clean[,!c(colnames(aug_clean) %in% c("sample_id","sample_type","trial_id"))]))*1000000
...
```{r}
dvd.n.perc = dvd.norm[,c("Daphnia_magna","Dikerogammarus_villosus")]/rowSums(dvd.norm[,c("Daphnia_magna","Dikerogammarus_villosus")])*100

dva.n.perc = dva.norm[,c("Asellus_aquaticus","Dikerogammarus_villosus")]/rowSums(dva.norm[,c("Asellus_aquaticus","Dikerogammarus_villosus")])*100

dvc.n.perc = dvc.norm[,c("Crangonyx_pseudogracilis","Dikerogammarus_villosus")]/rowSums(dvc.norm[,c("Crangonyx_pseudogracilis","Dikerogammarus_villosus")])*100
```

```{r}
print("A.aquaticus - D.villosus")
mean(dva.n.perc[,"Asellus_aquaticus"])
sd(dva.n.perc[,"Asellus_aquaticus"])/sqrt(length(dva.n.perc[,"Asellus_aquaticus"]))
mean(dva.n.perc[,"Dikerogammarus_villosus"])
sd(dva.n.perc[,"Dikerogammarus_villosus"])/sqrt(length(dva.n.perc[,"Dikerogammarus_villosus"]))

print("D.magna - D.villosus")
mean(dvd.n.perc[,"Daphnia_magna"])
sd(dvd.n.perc[,"Daphnia_magna"])/sqrt(length(dvd.n.perc[,"Daphnia_magna"]))
mean(dvd.n.perc[,"Dikerogammarus_villosus"])
sd(dvd.n.perc[,"Dikerogammarus_villosus"])/sqrt(length(dvd.n.perc[,"Dikerogammarus_villosus"]))

print("C.pseudogracilis - D.villosus")
mean(dvc.n.perc[,"Crangonyx_pseudogracilis"])
sd(dvc.n.perc[,"Crangonyx_pseudogracilis"])/sqrt(length(dvc.n.perc[,"Crangonyx_pseudogracilis"]))
mean(dvc.n.perc[,"Dikerogammarus_villosus"])
sd(dvc.n.perc[,"Dikerogammarus_villosus"])/sqrt(length(dvc.n.perc[,"Dikerogammarus_villosus"]))
```


Probit GLM on individual preys

```{r setting up data for probit}
unique(dva$trial_repl)
unique(dvd$trial_repl)
unique(dvc$trial_repl)

colnames(dva)

dva.det = as.data.frame(dva)
dvd.det = as.data.frame(dvd)
dvc.det = as.data.frame(dvc)

dva.det$time = sub('^\\w{3}', "", dva.det$trial_id)
dvd.det$time = sub('^\\w{3}', "", dvd.det$trial_id)
dvc.det$time = sub('^\\w{3}', "", dvc.det$trial_id)

## Asellus ##
dva.det = as.data.frame(dva.det[,!c(colnames(dva.det) %in% c("sample_id","trial_id","pcr_repl"))] %>% group_by(trial_repl, sample_type, time) %>% summarise_all(list(sum)))

## Daphnia ##
dvd.det = as.data.frame(dvd.det[,!c(colnames(dvd.det) %in% c("sample_id","trial_id","pcr_repl"))] %>% group_by(trial_repl, sample_type, time) %>% summarise_all(list(sum)))

## Crangonyx ##
dvc.det = as.data.frame(dvc.det[,!c(colnames(dvc.det) %in% c("sample_id","trial_id","pcr_repl"))] %>% group_by(trial_repl, sample_type, time) %>% summarise_all(list(sum)))

dva.det[,!c(colnames(dva.det) %in% c("sample_id","sample_type","trial_repl","pcr_repl","trial_id","time"))][dva.det[,!c(colnames(dva.det) %in% c("sample_id","sample_type","trial_repl","pcr_repl","trial_id","time"))] > 0] <- 1
dvd.det[,!c(colnames(dvd.det) %in% c("sample_id","sample_type","trial_repl","pcr_repl","trial_id","time"))][dvd.det[,!c(colnames(dvd.det) %in% c("sample_id","sample_type","trial_repl","pcr_repl","trial_id","time"))] > 0] <- 1
dvc.det[,!c(colnames(dvc.det) %in% c("sample_id","sample_type","trial_repl","pcr_repl","trial_id","time"))][dvc.det[,!c(colnames(dvc.det) %in% c("sample_id","sample_type","trial_repl","pcr_repl","trial_id","time"))] > 0] <- 1

dva.det$Asellus_aquaticus = dva.det$Asellus_aquaticus/3
dvd.det$Daphnia_magna = dvd.det$Daphnia_magna/5
dvc.det$Crangonyx_pseudogracilis = dvc.det$Crangonyx_pseudogracilis/5
```

We need to remove the control replicates before running the Probit.
 
```{r probit}
dva.det = dva.det[!c(dva.det$trial_repl == "control"),]
dvd.det = dvd.det[!c(dvd.det$trial_repl == "control"),]
dvc.det = dvc.det[!c(dvc.det$trial_repl == "control"),]

dva.det = as.data.frame(dva.det[,!c(colnames(dva.det) %in% c("trial_repl"))] %>% group_by(sample_type, time) %>% summarise_all(list(sum)))
dvd.det = as.data.frame(dvd.det[,!c(colnames(dvd.det) %in% c("trial_repl"))] %>% group_by(sample_type, time) %>% summarise_all(list(sum)))
dvc.det = as.data.frame(dvc.det[,!c(colnames(dvc.det) %in% c("trial_repl"))] %>% group_by(sample_type, time) %>% summarise_all(list(sum)))

dva.det = transform(dva.det, time=as.numeric(time))
dvd.det = transform(dvd.det, time=as.numeric(time))
dvc.det = transform(dvc.det, time=as.numeric(time))

dva.probit = glm(Asellus_aquaticus~time, family = quasibinomial(link="probit"), data=dva.det)
summary(dva.probit); drop1(dva.probit, test="Chi")

1-pchisq((dva.probit$null.deviance - dva.probit$deviance), df=(dva.probit$df.null - dva.probit$df.residual))

dvd.probit = glm(Daphnia_magna~time, family = quasibinomial(link="probit"), data=dvd.det)
summary(dvd.probit); drop1(dvd.probit, test="Chi")

1-pchisq((dvd.probit$null.deviance - dvd.probit$deviance), df=(dvd.probit$df.null - dvd.probit$df.residual))

dvc.probit = glm(Crangonyx_pseudogracilis~time, family = quasibinomial(link="probit"), data=dvc.det)
summary(dvc.probit); drop1(dvc.probit, test="Chi")

1-pchisq((dvc.probit$null.deviance - dvc.probit$deviance), df=(dvc.probit$df.null - dvc.probit$df.residual))

# Adding the predictions and se.
# Asellus
dva.new = data.frame(time=seq(from=min(dva.det$time), to=max(dva.det$time), by=1))
dva.pred = predict(dva.probit, newdata=dva.new, type="link", se=T)

plot(dva.new$time, exp(dva.pred$fit)/(1+exp(dva.pred$fit)), type="l", ylim=c(0,1), xlab="times (hours)", ylab="Ratio of predators positive to prey DNA", main="A. aquaticus")
lines(dva.new$time, exp(dva.pred$fit+1.96*dva.pred$se.fit)/(1+exp(dva.pred$fit+1.96*dva.pred$se.fit)), lty=2)
lines(dva.new$time, exp(dva.pred$fit-1.96*dva.pred$se.fit)/(1+exp(dva.pred$fit-1.96*dva.pred$se.fit)), lty=2)
points(dva.det$time, dva.det$Asellus_aquaticus)
rug(dva.det$time, side=1, col="black", ticksize = 0.02, lwd = 2)

# Daphnia
dvd.new = data.frame(time=seq(from=min(dvd.det$time), to=max(dvd.det$time), by=1))
dvd.pred = predict(dvd.probit, newdata=dvd.new, type="link", se=T)

plot(dvd.new$time, exp(dvd.pred$fit)/(1+exp(dvd.pred$fit)),type="l", ylim=c(0,1), xlab="times (hours)", ylab="Ratio of predators positive to prey DNA", main="D. magna")
lines(dvd.new$time, exp(dvd.pred$fit+1.96*dvd.pred$se.fit)/(1+exp(dvd.pred$fit+1.96*dvd.pred$se.fit)), lty=2)
lines(dvd.new$time, exp(dvd.pred$fit-1.96*dvd.pred$se.fit)/(1+exp(dvd.pred$fit-1.96*dvd.pred$se.fit)), lty=2)
points(dvd.det$time, dvd.det$Daphnia_magna)
rug(dvd.det$time, side=1, col="black", ticksize = 0.02, lwd = 2)

# Crangonyx
dvc.new = data.frame(time=seq(from=min(dvc.det$time), to=max(dvc.det$time), by=1))
dvc.pred = predict(dvc.probit, newdata=dvc.new, type="link", se=T)

plot(dvc.new$time, exp(dvc.pred$fit)/(1+exp(dvc.pred$fit)), type="l", ylim=c(0,1), xlab="times (hours)", ylab="Ratio of predators positive to prey DNA", main="C. pseudogracilis")
lines(dvc.new$time, exp(dvc.pred$fit+1.96*dvc.pred$se.fit)/(1+exp(dvc.pred$fit+1.96*dvc.pred$se.fit)), lty=2)
lines(dvc.new$time, exp(dvc.pred$fit-1.96*dvc.pred$se.fit)/(1+exp(dvc.pred$fit-1.96*dvc.pred$se.fit)), lty=2)
points(dvc.det$time, dvc.det$Crangonyx_pseudogracilis)
rug(dvc.det$time, side=1, col="black", ticksize = 0.02, lwd = 2)
```

```{r detection curves plot}
png("R_images/feeding_trials_probit.png", width=4000, height=4000, units="px", res=300)
...
dvd.trials = as.data.frame(dvd.norm[,!c(colnames(dvd.norm) %in% "pcr_repl")] %>% group_by(sample_id, sample_type, trial_repl, trial_id) %>% summarise_all(list(sum)))

dva.trials = as.data.frame(dva.norm[,!c(colnames(dva.norm) %in% "pcr_repl")] %>% group_by(sample_id, sample_type, trial_repl, trial_id) %>% summarise_all(list(sum)))

dvc.trials = dvc.norm[,!c(colnames(dvc.norm) %in% "pcr_repl")]
```


```{r setting the frames for plotting}
...
trials.melt$variable = gsub("[a-z]{6,}_", ".", trials.melt$variable)

str(trials.melt)

trials.melt = trials.melt[trials.melt$variable != "D.villosus",] %>% mutate(variable = factor(variable, levels=c("A.aquaticus","D.magna","C.pseudogracilis")))

png("R_images/feeding_trials_boxplot.png", width=5000, height=4000, units="px", res=300)
ggplot(trials.melt, aes(times, value)) +
  geom_boxplot(outlier.shape=NA) +
  geom_jitter(data=trials.melt[trials.melt$value > 0,], aes(times, value, color="red")) +
  geom_jitter(data=trials.melt[trials.melt$value == 0,], aes(times, value), alpha=0.4) +
  facet_grid(variable~., scale="free") +
  theme_bw() +
  labs(x="Digestion time (hours)",y="Normalised DNA reads") +
...
One season across 2 lakes: Wroxham Broad, and Grafham Water.
...
guts[guts$predator_id == "Dvil","Dikerogammarus_villosus"] = 0
guts[guts$predator_id == "Gtig","Gammarus_tigrinus"] = 0
guts = guts[,!c(colnames(guts) %in% "Dikerogammarus_villosus")]
```
...
```{r}
length(grep("WB", abund.guts$sample_id))
length(grep("GW", abund.guts$sample_id))

which(rowSums(abund.guts[,!c(colnames(abund.guts) %in% c("sample_id","sample_type","site","lake","predator_id"))]) > 0)
```


```{r rarefaction curves}
raref.guts = as.data.frame(abund.guts[grep("Dvil", abund.guts$predator_id),!c(colnames(abund.guts) %in% c("sample_id","sample_type","site"))] %>% group_by(lake, predator_id) %>% summarise_all(list(sum)))
...
web.guts = web.guts[!c(rownames(web.guts) %in% c("Gtig GW")),]
web.guts = t(web.guts)
...
```{r plot order}
seq.high=c("Dvil GW","Dvil WB")

seq.low=c("C.luridus","C.laricomalis","G.tigrinus","B.vejdovskyanum","C.brevilabris","Cricotopus","Cyclopidae","Daphnia","D.polymorpha","G.zaddachi","P.pediculus")

col.network=c("yellow","yellow","red","tan4","cyan3","yellow","gold3","cyan3","darkorchid3","red","cyan3")
```

```{r legend order}
name.multi=c("Amphipoda","Cladocera","Copepoda","Diptera","Mollusca","Oligochaeta")
col.multi=c("red","cyan3","gold3","yellow","darkorchid3","tan4")
```
...
# Unfortunately we need to remove D. villosus from eDNA dataset due to contamination.
edna[,!c(colnames(edna) %in% "Dikerogammarus_villosus")]
```
...
raref.edna = as.data.frame(abund.edna[,!c(colnames(abund.edna) %in% c("sample_id","sample_type","site"))] %>% group_by(lake) %>% summarise_all(list(sum)))

rownames(raref.edna) = raref.edna$lake
colnames(raref.edna)

raref.edna = t(raref.edna[,!c(colnames(raref.edna) %in% c("lake","site"))])

raref.edna_inext = iNEXT(raref.edna, q=0, datatype = "abundance", se=T, endpoint=150)

strip_lab=c("0"="Species Richness","1"="Shannon Diversity")

png("R_images/edna_rarefactions.png", width=4000, height=4000, units="px", res=300)
ggiNEXT(raref.edna_inext) +
  labs(y="N species", x="Number detections") +
  theme(legend.position="bottom") +
  facet_grid(order~., labeller=as_labeller(strip_lab), scales="free")
dev.off()
```

eDNA community site occupancy

```{r eDNA site occupancy}
occup.edna = as.data.frame(abund.edna[,!c(colnames(abund.edna) %in% c("sample_id","sample_type","site","lake"))]/6)
occup.edna = bind_cols(occup.edna, abund.edna[,c(colnames(abund.edna) %in% c("sample_id","sample_type","site","lake"))])
```
...
abund.kick = abund.kick[,!c(colnames(abund.kick) %in% "REPLICATE")]
```
...
raref.kick = abund.kick[,!c(colnames(abund.kick) %in% c("sample_id","sample_type"))]
...
Kick-samples site occupancy

```{r Kick-samples site occupancy}
...
adonis.kick = as.data.frame(occup.kick[,!c(colnames(occup.kick) %in% c("sample_id","sample_type","site","lake"))]/6)
adonis.kick = bind_cols(adonis.kick, occup.kick[,c(colnames(occup.kick) %in% c("sample_id","sample_type","site","lake"))])
```
...
```{r summary table for feeding trials}
dva.summary = as.data.frame(dva)
dvd.summary = as.data.frame(dvd)
dvc.summary = as.data.frame(dvc)

dva.summary = transform(dva.summary, pcr_repl=as.character(pcr_repl))
dvd.summary = transform(dvd.summary, pcr_repl=as.character(pcr_repl))
dvc.summary = transform(dvc.summary, pcr_repl=as.character(pcr_repl))

dva.summary$trial_id = gsub("^DvA", "", dva.summary$sample_id)
dvd.summary$trial_id = gsub("^DvD", "", dvd.summary$sample_id)
dvc.summary$trial_id = gsub("^DvG", "", gsub("[.]", "", dvc.summary$sample_id))

dva.summary = as.data.frame(dva.summary[,!c(colnames(dva.summary) %in% c("sample_id","pcr_repl"))] %>% group_by(sample_type, trial_id, trial_repl) %>% summarise_all(list(sum)))
dvd.summary = as.data.frame(dvd.summary[,!c(colnames(dvd.summary) %in% c("sample_id","pcr_repl"))] %>% group_by(sample_type, trial_id, trial_repl) %>% summarise_all(list(sum)))
dvc.summary = as.data.frame(dvc.summary[,!c(colnames(dvc.summary) %in% c("sample_id","pcr_repl"))] %>% group_by(sample_type, trial_id, trial_repl) %>% summarise_all(list(sum)))

feed.summary = full_join(dva.summary[,!c(colnames(dva.summary) %in% "Dikerogammarus_villosus")], dvd.summary[,!c(colnames(dvd.summary) %in% "Dikerogammarus_villosus")])
feed.summary = full_join(feed.summary, dvc.summary[,!c(colnames(dvc.summary) %in% "Dikerogammarus_villosus")])

feed.summary = arrange(feed.summary, trial_id)

colnames(feed.summary) = gsub("_", " ", colnames(feed.summary))

# Saving feeding trials output
write.csv(feed.summary, "R_outputs/Feeding-trials_summary_table.csv")
```

```{r summary table for wild samples}
guts.summary = as.data.frame(abund.guts[,!c(colnames(abund.guts) %in% c("sample_id"))] %>% group_by(sample_type,lake,predator_id,site) %>% summarise_all(list(sum)))
edna.summary = as.data.frame(abund.edna[,!c(colnames(abund.edna) %in% c("sample_id"))] %>% group_by(sample_type,lake,site) %>% summarise_all(list(sum)))
kick.summary = as.data.frame(abund.kick[,!c(colnames(abund.kick) %in% c("sample_id","REPLICATE"))] %>% group_by(sample_type,lake,site) %>% summarise_all(list(sum)))
...
```{r publication plot pcr replicates}
dva = dva %>% mutate(trial_id = factor(trial_id, levels=c("0","4","8","12","24","36"))) %>% mutate(trial_repl = factor(trial_repl, levels=c("control","1","2","3","4","5")))

dvd = dvd %>% mutate(trial_id = factor(trial_id, levels=c("0","4","8","12","24","36"))) %>% mutate(trial_repl = factor(trial_repl, levels=c("control","1","2","3","4","5")))

dvc = dvc %>% mutate(trial_id = factor(trial_id, levels=c("0","4","8","12","24","36"))) %>% mutate(trial_repl = factor(trial_repl, levels=c("control","1","2","3","4","5")))

fa = ggplot(dva, aes(pcr_repl, Asellus_aquaticus)) +
  geom_point(data=dva[c(dva$Asellus_aquaticus > 0),], inherit.aes=T, color="red") +
  geom_point(data=dva[c(dva$Asellus_aquaticus == 0),], inherit.aes=T, alpha=0.5) +
  labs(x="PCR replicates", y="DNA reads", subtitle="A. aquaticus trials replicates") +
  facet_grid(trial_id~trial_repl) +
  theme_bw() +
  guides(fill=guide_legend(ncol=1)) +
  theme(legend.position="none", legend.direction ="vertical", axis.text = element_text(size = 15), 
        strip.text = element_text(size=11), axis.title = element_text(size =15),
        legend.text = element_text(size=15))


fd = ggplot(dvd, aes(pcr_repl, Daphnia_magna)) +
 geom_point(data=dvd[c(dvd$Daphnia_magna > 0),], inherit.aes=T, color="red") +
  geom_point(data=dvd[c(dvd$Daphnia_magna == 0),], inherit.aes=T, alpha=0.5) +
  labs(x="PCR replicates", y="DNA reads", subtitle="D. magna trials replicates") +
  facet_grid(trial_id~trial_repl) +
  theme_bw() +
  guides(fill=guide_legend(ncol=1)) +
  theme(legend.position="none", legend.direction ="vertical", axis.text = element_text(size = 15), 
...
fc = ggplot(dvc, aes("1", Crangonyx_pseudogracilis)) +
  geom_point(data=dvc[c(dvc$Crangonyx_pseudogracilis > 0),], inherit.aes=T, color="red") +
  geom_point(data=dvc[c(dvc$Crangonyx_pseudogracilis == 0),], inherit.aes=T, alpha=0.5) +
  labs(x="PCR replicates", y="DNA reads", subtitle="C. pseudogracilis trials replicates") +
  facet_grid(trial_id~trial_repl) +
  theme_bw() +
  guides(fill=guide_legend(ncol=1)) +
  theme(legend.position="none", legend.direction ="vertical", axis.text = element_text(size = 15), 
...
png("R_images/publication_pcr_replicates.png", width=4000, height=4000, units="px", res=300) 
plot_grid(fa, fd, fc, labels=c("a)","b)","c)"), nrow=1, ncol=3)
dev.off()
```

```{r feeding trials publication}
png("R_images/publication_feeding_trials.png", width=5000, height=4000, units="px", res=300)
ggplot(trials.melt, aes(times, value)) +
  geom_boxplot(outlier.shape=NA) +
  geom_jitter(data=trials.melt[trials.melt$value > 0,], aes(times, value, color="red", size=12)) +
  geom_jitter(data=trials.melt[trials.melt$value == 0,], aes(times, value, size=12), alpha=0.4) +
  facet_grid(variable~., scale="free") +
  theme_bw() +
  labs(x="Digestion time (hours)",y="Normalised DNA reads") +
...
```{r publication bipartite}
# Producing the bipartite network
png("R_images/publication_bipartite_network.png", width=4000, height=2500, units="px", res=300)
plotweb(sortweb(web.guts, sort.order="seq", sequence=list(seq.higher=seq.high, seq.lower=seq.low)), method = "normal", y.width.low=0.1, y.width.high=0.08,	y.lim=c(0,3), labsize=2, low.lab.dis=0.05, high.lab.dis=0.1, low.spacing=0.012, high.spacing = 0.11, text.rot=90, col.high="black", col.low=col.network, high.y=2.5, low.y=1.5)
# Producing the legend
legend("bottom", legend=name.multi, fill=col.multi, border="black", bty="n", ncol=3, xpd=T, cex=1.7)
dev.off()
...
```{r barplot summary}
final.melt = full_join(guts, edna)
final.melt = full_join(final.melt, abund.kick)

final.melt = melt(final.melt)

unique(final.melt$variable)

final.melt$order = NA
final.melt[grep("Limnesia", final.melt$variable),"order"] = "Acaria"
final.melt[grep("Corophium|Crangonyx|Gammarus|Dikerogammarus", final.melt$variable),"order"] = "Amphipoda"
final.melt[grep("Bothrioneurum|Chaetogaster|Stylaria|Lumbricillus|Nais|Ophidonais|Uncinais|Tubificida", final.melt$variable),"order"] = "Annelida"
final.melt[grep("Chydorus|Daphnia|Polyphemus|Chydoridae|Daphniidae|Diaphanosoma|Eubosmina|Macrothrix|Eurycercus", final.melt$variable),"order"] = "Cladocera"
final.melt[grep("Dytiscidae|Platambus|Haliplidae|Elmidae|Limnius", final.melt$variable),"order"] = "Coleoptera"
final.melt[grep("Eudiaptomus|Eurytemora|Cyclopidae|Acanthocyclops|Cyclopoida|Cyclops|Eucyclops|Mesocyclops", final.melt$variable),"order"] = "Copepoda"
final.melt[grep("Chironomus|Cricotopus|Chironomidae|Diptera|Polypedilum|Procladius|Tanytarsus|Dolichopodidae|Nematocera|Tipulidae|Brachycera|Ceratopogonidae", final.melt$variable),"order"] = "Diptera"
final.melt[grep("Cloeon|Caenis", final.melt$variable),"order"] = "Ephemeroptera"
final.melt[grep("Chaetonotus", final.melt$variable),"order"] = "Gastrotricha"
final.melt[grep("Corixidae|Corixa|Micronecta|Notonecta|Velliidae|Plea|Nepa", final.melt$variable),"order"] = "Hemiptera"
final.melt[grep("Helobdella|Glossiphonia|Placobdella", final.melt$variable),"order"] = "Hirudinida"
final.melt[grep("Hydra", final.melt$variable),"order"] = "Hydrozoa"
final.melt[grep("Asellus", final.melt$variable),"order"] = "Isopoda"
final.melt[grep("Dreissena|Pisidium|Sphaerium|Unio|Radix|Potamopyrgus|Stagnicola|Bithynia|Lymnaea|Lymnaeidae|Planorbis|Valvata|Bithyniidae|Theodoxus|Segmentina|Valvatidae", final.melt$variable),"order"] = "Mollusca"
final.melt[grep("Plectus|Polyarthra", final.melt$variable),"order"] = "Nematoda"
final.melt[grep("Erythromma|Coenagrionidae", final.melt$variable),"order"] = "Odonata"
final.melt[grep("Cyprideis|Cypridopsis|Limnocythere", final.melt$variable),"order"] = "Ostracoda"
final.melt[grep("Stenostomum", final.melt$variable),"order"] = "Platyhelminthes"
final.melt[grep("Asplanchna|Euchlanis|Brachionus|Keratella|Philodina|Ploima|Synchaeta", final.melt$variable),"order"] = "Rotifera"
final.melt[grep("Lype|Leptoceridae|Ecnomus|Oecetis|Ceraclea", final.melt$variable),"order"] = "Trichoptera"

# 20 colours for the Orders
cols_barplot = c("red","tan4","cyan3","greenyellow","gold3","yellow","lightpink1","burlywood","deeppink","darkseagreen","darkcyan","blue","darkorchid","darkorange","olivedrab","grey48","darkgreen","khaki3")

png("R_images/summary_barplot.png", height=4000, width=4000, units="px", res=300)
final.melt %>% mutate(sample_type = factor(sample_type, levels=c("gut contents","eDNA","kick"))) %>% mutate(lake = factor(lake, levels=c("GW","WB","RB"))) %>%
  ...
  scale_y_continuous(expand=c(0,0), breaks=c(seq(0,1,0.1))) +
  facet_grid(.~lake) +
  labs(x="", y="composition (ratio)") +
  guides(fill=guide_legend(ncol=1)) +
  theme(legend.position="right", legend.direction ="vertical", axis.text = element_text(size = 15), 
        strip.text = element_text(size=15), axis.title = element_text(size =15),
        legend.text = element_text(size=15))
dev.off()
```

```{r publication probit plot}
png("R_images/publication_feeding_trials_probit.png", width=4000, height=4000, units="px", res=300)
par(mfrow=c(3,1), mar=c(5,6,3,1))
plot(dva.new$time, exp(dva.pred$fit)/(1+exp(dva.pred$fit)), type="l", ylim=c(0,1), xlab="times (hours)", ylab="Probability of detection (A. aquaticus)", cex.axis=2, cex.lab=2)
lines(dva.new$time, exp(dva.pred$fit+1.96*dva.pred$se.fit)/(1+exp(dva.pred$fit+1.96*dva.pred$se.fit)), lty=2, lwd=1)
lines(dva.new$time, exp(dva.pred$fit-1.96*dva.pred$se.fit)/(1+exp(dva.pred$fit-1.96*dva.pred$se.fit)), lty=2, lwd=1)
points(dva.det$time, dva.det$Asellus_aquaticus, cex=2)

plot(dvd.new$time, exp(dvd.pred$fit)/(1+exp(dvd.pred$fit)), type="l", ylim=c(0,1), xlab="times (hours)", ylab="Probability of detection (D. magna)", cex.axis=2, cex.lab=2)
lines(dvd.new$time, exp(dvd.pred$fit+1.96*dvd.pred$se.fit)/(1+exp(dvd.pred$fit+1.96*dvd.pred$se.fit)), lty=2)
lines(dvd.new$time, exp(dvd.pred$fit-1.96*dvd.pred$se.fit)/(1+exp(dvd.pred$fit-1.96*dvd.pred$se.fit)), lty=2)
points(dvd.det$time, dvd.det$Daphnia_magna, cex=2)

plot(dvc.new$time, exp(dvc.pred$fit)/(1+exp(dvc.pred$fit)), type="l", ylim=c(0,1), xlab="times (hours)", ylab="Probability of detection (C. pseudogracilis)",cex.axis=2, cex.lab=2)
lines(dvc.new$time, exp(dvc.pred$fit+1.96*dvc.pred$se.fit)/(1+exp(dvc.pred$fit+1.96*dvc.pred$se.fit)), lty=2)
lines(dvc.new$time, exp(dvc.pred$fit-1.96*dvc.pred$se.fit)/(1+exp(dvc.pred$fit-1.96*dvc.pred$se.fit)), lty=2)
points(dvc.det$time, dvc.det$Crangonyx_pseudogracilis, cex=2)
dev.off()
```
