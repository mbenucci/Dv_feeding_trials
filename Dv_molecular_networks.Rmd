...
rownames(occup.kick) = paste(occup.kick$sample_id, occup.kick$time, sep="_")

adonis.kick = as.data.frame(occup.kick)
adonis.kick[,!c(colnames(adonis.kick) %in% c("sample_id","site","sample_type","lake"))][adonis.kick[,!c(colnames(adonis.kick) %in% c("sample_id","site","sample_type","lake"))] > 0] <- 1
...
adonis(adonis.kick~fact_lake.kick*fact_site.kick, permutations = 9999, method="jaccard")
```
...
# NOTE TO SOLVE NA in SPECIES RICHNESS PLOT
