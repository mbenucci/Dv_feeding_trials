...
dva.probit = glm(Asellus_aquaticus~time, family = quasibinomial(link="logit"), data=dva.det)
summary(dva.probit); drop1(dva.probit, test="Chi")

dvd.probit = glm(Daphnia_magna~time, family = quasibinomial(link="logit"), data=dvd.det)
summary(dvd.probit); drop1(dvd.probit, test="Chi")

dvc.probit = glm(Crangonyx_pseudogracilis~time, family = quasibinomial(link="logit"), data=dvc.det)
summary(dvc.probit); drop1(dvc.probit, test="Chi")

plot(dva.probit)
qqnorm(resid(dva.probit,type="deviance"))
qqline(resid(dva.probit,type="deviance"),col="red",lwd=2)
hist(resid(dva.probit,type="deviance"),freq=F)

## FIX HERE!
## PLOTTING THE MODELS. WE USE "LINK" IN ORDER TO KEEP PREDICTIONS ##
## AS 1 and 0 ##

dvd.new = data.frame(time=seq(from=min(dvd.det$time),to=max(dvd.det$time),by=1))
dvd.pred = predict(dvd.probit, newdata=as.matrix(dvd.new), type="link", se=T)

new.asellus = data.frame(times=seq(from=min(asellus.glm$times),
to=max(asellus.glm$times),by=1))
asellus.pred = predict(asellus.glm.model, newdata=new.asellus, type="link", se=T)

new.crangonyx = data.frame(times=seq(from=min(crangonyx.glm$times),
to=max(crangonyx.glm$times),by=1))
crangonyx.pred = predict(crangonyx.glm.prob, newdata=new.crangonyx, type="link", se=T)

dev.new()
par(mfrow=c(3,1))
par(mar=c(5,6,3,1))

plot(dvd.new$times, exp(dvd.pred$fit)/(1+exp(dvd.pred$fit)),
type="l", ylim=c(0,1), xlab="times (hours)", ylab="Ratio of predators positive
to prey DNA", main="Daphnia magna")
lines(dvd.new$times, exp(dvd.pred$fit+1.96*dvd.pred$se.fit)/
(1+exp(dvd.pred$fit+1.96*dvd.pred$se.fit)), lty=2)
lines(dvd.new$times, exp(dvd.pred$fit-1.96*dvd.pred$se.fit)/
(1+exp(dvd.pred$fit-1.96*dvd.pred$se.fit)), lty=2)
points(dvd.det$time, dvd.det$valu)
...
plot(new.asellus$times, exp(asellus.pred$fit)/(1+exp(asellus.pred$fit)),
type="l", ylim=c(0,1), xlab="times (hours)", ylab="Ratio of predators positive
to prey DNA", main="Asellus aquaticus")
lines(new.asellus$times, exp(asellus.pred$fit+1.96*asellus.pred$se.fit)/
(1+exp(asellus.pred$fit+1.96*asellus.pred$se.fit)), lty=2)
lines(new.asellus$times, exp(asellus.pred$fit-1.96*asellus.pred$se.fit)/
(1+exp(asellus.pred$fit-1.96*asellus.pred$se.fit)), lty=2)
points(asellus.glm$times, asellus.glm$value)
rug(asellus.glm$times, side=1, col="black", ticksize = 0.02, lwd = 2)

#dev.new()
#par(mfrow=c(2,2))
#par(mar=c(5,6,3,1))

plot(new.crangonyx$times, exp(crangonyx.pred$fit)/(1+exp(crangonyx.pred$fit)),
type="l", ylim=c(0,1), xlab="times (hours)", ylab="Ratio of predators positive
to prey DNA", main="Crangonyx pseudogracilis")
lines(new.crangonyx$times, exp(crangonyx.pred$fit+1.96*crangonyx.pred$se.fit)/
(1+exp(crangonyx.pred$fit+1.96*crangonyx.pred$se.fit)), lty=2)
lines(new.crangonyx$times, exp(crangonyx.pred$fit-1.96*crangonyx.pred$se.fit)/
(1+exp(crangonyx.pred$fit-1.96*crangonyx.pred$se.fit)), lty=2)
points(crangonyx.glm$times, crangonyx.glm$value)
rug(crangonyx.glm$times, side=1, col="black", ticksize = 0.02, lwd = 2)
