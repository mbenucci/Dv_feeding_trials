...
feb_clean[,grep("POS", colnames(feb_clean))]
...
# 7. Prey detections in gut contents
...
Removing taxa that are equal to zero. We still have some contamination from PCR, labeles as `cont.17` till `cont.20`. Three of those contain _D.villosus_ DNA only (N=3244, N=38246, N=40021), while one containes _D.villosus_ (N=3304), _Osmia bicornis_ (N=867), Hymenoptera (N=5), _Homo sapiens_ (N=2), _Ephemera danica_ (N=2).
...
```{r}
guts[c(rownames(guts) %in% c("cont.17","cont.18","cont.19","cont.20")),][guts[c(rownames(guts) %in% c("cont.17","cont.18","cont.19","cont.20")),] > 0]

guts["cont.17",][guts["cont.17",] > 0]
guts["cont.18",][guts["cont.18",] > 0]
guts["cont.19",][guts["cont.19",] > 0]
guts["cont.20",][guts["cont.20",] > 0]

guts = guts[!c(rownames(guts) %in% c("cont.17","cont.18","cont.19","cont.20")),]
```

```{r removing positive taxa}
guts = guts[,!c(colnames(guts) %in% c("Osmia_bicornis","Harmonia_axyridis","Homo_sapiens"))]


```

# NOTE: from here but need fixing in clean up!!!
